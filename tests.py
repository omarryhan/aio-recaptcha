...
def test_html():
    assert (
        aiorecaptcha.html()
        == '<div class="g-recaptcha" data-theme="light" data-size="normal" data-type="image"></div>'
    )
...
def test_js_fails_on_extra_kwargs():
    with pytest.raises(TypeError) as e:
        aiorecaptcha.js(onload="asd", asd="asd")
        assert "Extra kwargs" in str(e) and "asd" in str(e) and "onload" not in str(e)


def test_html_fails_on_extra_kwargs():
    with pytest.raises(TypeError) as e:
        aiorecaptcha.html(site_key="asd", asd="asd")
        assert "Extra kwargs" in str(e) and "asd" in str(e) and "site_key" not in str(e)
...
def test_verify(event_loop):
    ...
    HTML = aiorecaptcha.html(
        site_key=TESTING_SITE_KEY, theme="dark", callback="verifyCallback"
    )
    ...
    async def verify(request):
        try:
            ...
            await aiorecaptcha.verify(
                secret=TESTING_SECRET_KEY, response=resp, remoteip=request.ip
            )
        finally:
            app.stop()
